<template>
  ...
  >
    <div ref="containerRef">
      ...
      >
        ...
        >
          ...
          >
            <img
              v-if="currentShowMessage.avatar"
              :src="currentShowMessage.avatar"
              style="width: 74px"
              alt=""
            />
            <img
              v-else
              src="@/assets/acount/defaultUser.png"
              style="width: 74px"
              alt=""
            />
          </div>
          ...
          >
            ...
            <div class="info">
              ...
              <span class="infoContent">{{
                currentShowMessage.accountName
              }}</span>
            </div>
            <div class="info">
              <span>手机号码: </span>
            ...
            <div class="info">
              <span>账号角色: </span>
              <span class="infoContent">{{
                getUserTypeRole(currentShowMessage.type)
              }}</span>
            </div>
          </div>
          <div
          ...
          >
            ...
            <div class="info">
              <span>性别: </span>
              <span class="infoContent">{{
                gender(currentShowMessage.sex)
              }}</span>
            </div>
            <div class="info">
              <span>年龄: </span>
              <span class="infoContent">{{
                getAgeByBirthday(currentShowMessage.birthday)
              }}</span>
            </div>
          </div>
          <div
          ...
          >
            ...
            <div class="info">
              <span style="vertical-align: text-bottom">本人照片: </span>
              <img
                v-if="currentShowMessage.userFace"
                @click="openUserMessage(3)"
                src="@/assets/acount/lookPng.png"
                style="width: 16px; cursor: pointer"
                alt=""
              />
              <span style="vertical-align: text-bottom" v-else>暂无</span>
              <!-- <template v-else>
                <span
                  v-if="isSelf"
            ...
            <div class="info">
              <span style="vertical-align: text-bottom">声纹: </span>
              <img
                v-if="currentShowMessage.userVoice"
                @click="openUserMessage(1)"
                src="@/assets/acount/playPng.png"
                style="width: 16px; cursor: pointer"
                alt=""
              />
              <span style="vertical-align: text-bottom" v-else>暂无</span>
              <!-- <template v-else>
                <span
                  v-if="isSelf"
                  @click="openFirstPhoneAuth(false)"
                  class="blue-color"
                  >去录制 ></span
                >
                <span v-else>暂无</span>
            ...
            <div class="info">
              <span style="vertical-align: text-bottom">身份证照片: </span>
              <img
            ...
            <div class="info">
              <span>身份证号: </span>
              <div
                v-if="currentShowMessage.userIdcard?.idcard"
                class="infoContent"
              >
        ...
        <div
          id="part2"
          style="width: 518px; height: 530; padding: 15px; overflow: auto"
          class="part part2Class"
        >
          ...
          <div class="tagsBox">
            <span
              class="tags"
              v-for="(item, index) in getSkillOrBusiness(
                currentShowMessage.tags,
                1
              )"
              :key="index"
            >
      ...
      <div
        style="
          background: #ffffff;
          box-shadow: 0px 5px 17px 0px rgba(48, 123, 214, 0.12);
          border-radius: 20px 20px 20px 20px;
          padding: 15px;
          margin: 16px 0;
        "
        id="part3"
        class="part part3Class"
      >
            ...
            <div class="tabContent">
              ...
              >
                <div :class="['Item', selected == 1 ? 'Itembg' : '']">
                  <img
                    src="@/assets/zuoxi/youxiu.png"
                    alt=""
                    @click="selectedCheck(1)"
                  />
                </div>
                <div
                  style="
                ...
                >
                  {{
                    getAwards(currentShowMessage.awards, 1).length > 0
                      ? dateFormat(
                          getAwards(currentShowMessage.awards, 1)[0]
                            .validTimeEnd,
                          "YYYY-MM"
                        )
                      : ""
              ...
              >
                <div :class="['Item', selected == 2 ? 'Itembg1' : '']">
                  <img
                    src="@/assets/zuoxi/xingji.png"
                    alt=""
                    @click="selectedCheck(2)"
                  />
                </div>
                <div
                  style="
      ...
      >
        ...
        <div class="part4Content">
          <div class="part4content-item">
            <div class="tabBox">
              <span
                :class="['tab', activeStudy == 1 ? 'active' : '']"
                @click="studyClick(1)"
                >总览
              </span>
              <span
                :class="['tab', activeStudy == 2 ? 'active' : '']"
                @click="studyClick(2)"
                >优秀录音</span
              >
            </div>
    ...
    <userMessageC
      ref="userMessageRef"
      :currentShowMessage="currentShowMessage"
    />
...
// 获取项目技能或擅长业务
const getAwards = (awards: any, type: number) => {
  let arr = [];
  if (awards && awards.length > 0) {
    arr = awards.filter((ele: any) => ele.type == type);
  }
  return arr;
};
// 获取个人信息
...
.part3Class {
  ...
  .partContent {
    ...
    .tabContent-item {
      ...
      .Itembg {
        background: url(@/assets/zuoxi/youxiuSelect.png) no-repeat center center /
          200%;
      }
      .Itembg1 {
        background: url(@/assets/zuoxi/xingjiSelect.png) no-repeat center center /
          200%;
      }
    }
    .contentItem-title {
...
.skills-item {
  background: linear-gradient(180deg, #eaf1ff 0%, rgba(255, 255, 255, 0.3) 61%);
  border-radius: 16px 16px 16px 16px;
  width: 470px;
  min-height: 105px;
  // background: #ffffff;
